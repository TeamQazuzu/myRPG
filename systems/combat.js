...
class CombatEngine {
    constructor() {
        ...
        this.enemyUnits = [];
        this.defenseBoost = 0; // 记录玩家本回合防御加成
    }
    // 启动战斗
    ...
    processTurn() {
        ...
        if (currentUnit.type === 'player') {
            ...
            this.isPlayerTurn = false;
            setTimeout(() => { this.enemyTurn(currentUnit.unit); }, 500);
        }
    }
    // 等待玩家操作
    waitForPlayerAction() {
    ...
    playerAction(action, target) {
        ...
        if (action === 'defend') {
            this.defenseBoost = 5;
            this.playerUnit.defense = (this.playerUnit.defense || 0) + this.defenseBoost;
            const msg = `${this.playerUnit.name} 进入防御姿态，防御力+5`;
            this.combatLog.push(msg);
            const event = new CustomEvent('combat-update', { detail: { combat: this, log: msg } });
            document.dispatchEvent(event);
            this.nextTurn();
            return;
        }
        ...
        if (action === 'item') {
            const msg = `${this.playerUnit.name} 使用道具`;
            this.combatLog.push(msg);
            const event = new CustomEvent('combat-update', { detail: { combat: this, log: msg } });
            document.dispatchEvent(event);
            this.nextTurn();
    ...
    enemyAI(enemy) {
        ...
        if (hpRatio < 0.2) return Math.random() < 0.3 ? 'skill' : 'attack';
        if (hpRatio < 0.5) return Math.random() < 0.5 ? 'skill' : 'attack';
    ...
    executeAction(unit, action, target) {
        ...
        switch(action) {
            case 'attack':
                ...
                break;
            case 'skill':
                damage = Math.floor(this.calculateDamage(unit, target) * 1.5);
                target.hp = Math.max(0, target.hp - damage);
        ...
        const event = new CustomEvent('combat-update', { detail: { combat: this, log: logMessage } });
        document.dispatchEvent(event);
        if (this.checkBattleEnd()) return;
        this.nextTurn();
    }
    ...
    nextTurn() {
        ...
        if (this.currentTurn >= this.turnOrder.length) {
            ...
            this.maxTurns--;
            // 清除玩家防御加成
            if (this.defenseBoost > 0 && this.playerUnit) {
                this.playerUnit.defense = Math.max(0, (this.playerUnit.defense || 0) - this.defenseBoost);
                this.defenseBoost = 0;
            }
            if (this.maxTurns <= 0) {
    ...
    endCombat(result) {
        ...
        let message = '';
        switch(result) {
            case 'player_victory': message = '🎉 战斗胜利！'; console.log('[战斗] 玩家胜利'); break;
            case 'player_defeat': message = '💀 战斗失败...'; console.log('[战斗] 玩家失败'); break;
            case 'timeout': message = '⏰ 战斗超时，平局！'; console.log('[战斗] 战斗超时'); break;
            default: message = '战斗结束';
        }
        this.combatLog.push(message);
        const event = new CustomEvent('combat-end', { detail: { combat: this, result: result } });
        document.dispatchEvent(event);
    }
    // 获取玩家单位
