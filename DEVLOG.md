...
- **开发状态**：v1.1 功能扩展，商店/对话/技能/掉落/面板交互已完成，灰烟村11帧可探索，处于可玩原型阶段
...
systems/combat.js      ← 战斗引擎（回合制、3v6、攻击/技能/防御/道具/撤退、伤害计算、HP同步回写、死亡处理、装备掉落）
...
systems/skills.js      ← 技能系统（技能数据/可用性检查/MP消耗/冷却/伤害计算/状态效果）
systems/scene.js       ← 帧式场景管理（灰烟村11帧/敌人数据/战斗触发/采集/休息/NPC对话）
ui/renderer.js         ← UI渲染（冒险/战斗/面板/技能选择/物品详情/商店/对话/搜索筛选）
...
### 高优先级
- [ ] 技能系统完善（四系伤害计算/异常状态效果/DOT/状态图标显示）
- [ ] 更多灰烟村场景帧（裁缝铺/皮匠铺/村医屋/墓地）
- [ ] 挂机采集系统（单帧单动作循环/结算界面/离线计算）
- [ ] 随从AI策略（攻击型/防御型/治疗型）
- [ ] 四种伤害类型完整实现（物理/火焰/冰霜/雷电）
...
## 已知问题（需修复）
1. **四系伤害未完整实现**：技能系统已有框架，但战斗中伤害类型（物理/火焰/冰霜/雷电）和异常状态（流血/灼烧/减速/僵直）的完整逻辑尚未接入
2. **挂机采集未实现**：场景中有挂机采集按钮，仍为占位提示
3. **随从AI单一**：随从只会普通攻击，无策略分支
4. **面板交互边界**：背包/装备面板操作后偶现状态不同步，需测试
5. **商店价格浮动未实现**：当前为固定价格，GDD 要求价格浮动
