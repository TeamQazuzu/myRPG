...
- **开发状态**：v2.2 队友经验共享 + 掉落调优 + 杂货店买卖 + 地下城副本 + 村长挑战调整
...
- [x] 灰烟村·地下城入口（副本第1层 Lv.4-6：骷髅兵×3）✅ v2.2
- [x] 灰烟村·地下城深处（副本第2层 Lv.7-9：地牢蜘蛛+骷髅弓手）✅ v2.2
- [x] 灰烟村·地下城底层（副本第3层 Lv.10-12：地牢守卫+骷髅法师+地牢食尸鬼）✅ v2.2
...
3. **浏览器缓存**：开发时修改JS文件后需手动刷新（已通过 `index.html` 的 `?v=22` 版本号缓解，后续发版需更新版本号）
...
### 2026-07-26 v2.2 队友经验共享 + 掉落调优 + 杂货店买卖 + 地下城副本 + 村长挑战调整

**Bug修复：队友经验不共享**
- 修复：`CompanionSystem.addExp` 已定义但从未被调用，队友无法获得经验导致不升级
  - `combat.js` 战斗结算时为每个队友调用 `addExp`（与玩家等量经验，受等级上限约束）
  - `elite.js` 精英怪战斗结算时同步队友经验
  - `quest.js` 任务奖励经验同步队友
  - `app.js` 离线经验结算同步队友
- 修复：`CompanionSystem.addExp` 兼容老存档字段缺失（exp/expToNext/maxLevel/isCompanion 兜底初始化）
- 修复：队友等级受玩家守门员进度约束（`StateUtils.getLevelCap`），防止队友超过当前等级上限
- `core/state.js` 默认队友数据补全缺失字段
- `ui/renderer.js` 队伍面板新增队友EXP进度条

**掉落概率调优**
- `combat.js` 装备掉落率下调：普通怪 15%→8%、精英怪 50%→25%、Boss 100%不变
- `gem.js` 宝石掉落率下调：普通怪 8%、精英怪 25%、Boss 50%（修复注释与代码不一致）
- 所有掉落增加背包容量检查，满时提示「背包已满，未获取掉落」
- 移除 Boss 战中冗余的 100% 装备掉落逻辑（避免背包满时产生「假掉落」）

**杂货店买卖系统**
- `shop.js` 提取 `getSellPrice(item)` 卖价计算函数（按品质和等级定价）
- `dialogue.js` 商店UI新增「购买」「卖出」标签页切换
  - 打开商店默认购买页
  - 卖出页显示背包物品列表 + 单件卖出/全部卖出按钮
- `css/style.css` 新增商店标签页和卖出按钮样式

**村长挑战调整**
- `scene.js` 村长不再要求 20 级才能挑战，随时可挑战
  - 挑战失败回酒馆（Boss战已有失败处理）
  - 北山古道传送门仍锁定（需击败村长）→「挑战不过就出不了村」
  - 其他守门员（守夜人/机械守卫/隐者）仍保持等级上限要求

**灰烟村地下城副本（Lv.4-12，填补早期经验缺口）**
- `scene.js` 新增 3 层地下城场景：
  - 地下城入口（Lv.4-6）：骷髅兵×3，可探索
  - 地下城深处（Lv.7-9）：地牢蜘蛛+骷髅弓手，可探索
  - 地下城底层（Lv.10-12）：地牢守卫+骷髅法师+地牢食尸鬼，可探索
- 新增 6 种副本怪物（骷髅兵/骷髅弓手/地牢蜘蛛/骷髅法师/地牢守卫/地牢食尸鬼）
- 探索逻辑新增 `greyVillage_dungeon` 区域（专属资源/稀有资源/遇敌池）
- 灰烟村枢纽 exits 新增「地下城入口」出口
- `core/data.js` 世界地图 greyVillage 地点列表同步更新

**其他改动**
- `index.html` 版本号 `?v=21` → `?v=22`

**文件变更**
- 修改文件：`systems/companions.js`、`systems/combat.js`、`systems/elite.js`、`systems/quest.js`、`systems/gem.js`、`systems/shop.js`、`systems/dialogue.js`、`systems/scene.js`、`core/state.js`、`core/data.js`、`ui/renderer.js`、`css/style.css`、`app.js`、`index.html`、`DEVLOG.md`
